...
FROM models JOIN brands ON (models.brand_id = brands.brand_id)
WHERE models.brand_id = 'for' AND models.name = 'Mustang';
...
SELECT * FROM models WHERE model_id between 15 AND 24  ORDER BY model_id ASC;
...
FROM models
LEFT JOIN awards ON (models.model_id = awards.winner_id)
...
FROM brands
LEFT JOIN models ON (brands.brand_id = models.brand_id)
...
FROM models AS m
JOIN (SELECT founded FROM brands GROUP BY founded) AS sub ON (sub.founded = m.year);